...
class Simulation:
    ...
    def _from_data_dict(cls, data: Dict[str, Any]) -> "Simulation":
        ...
        config.massive_masses = [
            float(bodies_massive[name]) for name in config.massive_objects
        ]

        unknown_radius_bodies = set(bodies_collision_radius) - set(
            config.massive_objects
        )
    ...
    def create_simulation(
        self, use_reboundx: bool = True, in_frame: str | None = None
    ) -> Rebound:
    ...
    def propagate(
        ...
        particle_hashes = Rebound.TEST_HASH_INIT + np.arange(
            n_particles, dtype=np.int64
        )
    ...
    def run(
        ...
        if populations is not None:
            ...
            for pop_config in populations:
                pop_states, pop_birth_times, _ = realise_population(
                    pop_config, self.rng
                )
