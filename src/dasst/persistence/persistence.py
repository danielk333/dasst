...
class Persistence:
    ...
    def save(self, obj: object) -> NoReturn:
        ...
        for type_, converter, ID in PERSISTENT_OBJECTS.values():
            if isinstance(obj, type_):
                ...
                bytes_data = ID.to_bytes(
                    Persistence.ID_len, byteorder="big", signed=False
                )
                bytes_data += len(obj_bytes).to_bytes(
                    Persistence.SIZE_len, byteorder="big", signed=False
                )
    ...
    def load(self, index=None):
        ...
        while True:
            byte_data = self.load_bytes(offset=offset, size=Persistence.HEAD_len)
            offset += Persistence.HEAD_len

            if len(byte_data) == 0:
                break

            converter_ID = int.from_bytes(
                byte_data[: Persistence.ID_len], byteorder="big", signed=False
            )
    ...
    def _convert_bytes(self, converter_ID: int, byte_data: bytes) -> object:
        for type_, converter, ID in PERSISTENT_OBJECTS.values():
            if converter_ID == ID:
                logger.debug(
                    f'{self.__class__}: converter "{repr(converter)}" and type "{repr(type_)}"'
                )
