...
def D_SH(orb_a, orb_b):
    ...
    Iabsin = (2*np.sin((orb_b.i - orb_a.i)*0.5))**2 + np.sin(orb_a.i)*np.sin(orb_b.i)*(2*np.sin((orb_b.Omega - orb_a.Omega)*0.5))**2
    I_ab = np.arcsin(0.5*np.sqrt(Iabsin))*2
    PI_ab = orb_b.omega - orb_a.omega + 2*np.arcsin(np.cos((orb_b.i - orb_a.i)*0.5)*np.sin((orb_b.Omega - orb_a.Omega)*0.5)/np.cos(I_ab*0.5))
...
def D_V(orb_a, orb_b, weights = None):
    ...
    lrl_a = np.cross(orb_a.cartesian[3:,...], c_a, axis=0)/mu_a - orb_a.cartesian[3:,...]/ra_norm

    rb_norm = np.linalg.norm(orb_b.cartesian[3:,...], axis=0)
    lrl_b = np.cross(orb_b.cartesian[3:,...], c_b, axis=0)/mu_b - orb_b.cartesian[3:,...]/rb_norm
